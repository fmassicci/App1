...
public class FormTextBox : TextBox, IFormControl
{
    ...
    public TextDataType DataType
    {
        get
        {
            return (TextDataType)GetValue(DataTypeProperty);
        }
        set
        {
    ...
    private void ApplyTextFormat()
    {
        switch (DataType)
        {
            case TextDataType.Single:
                Single.TryParse(Text, out Single s);
                FormattedText = s.ToString(Format);
                break;
    ...
    protected override void OnLostFocus(RoutedEventArgs e)
    {
        ...
        {
            case TextDataType.Single:
                if (!Single.TryParse(Text, out Single s))
                {
                    Text = "0";
                }
                break;
    ...
    private void OnBeforeTextChanging(TextBox sender, TextBoxBeforeTextChangingEventArgs args)
    {
        ...
        {
            case TextDataType.Single:
                args.Cancel = !Single.TryParse(str, out Single s);
                break;
            case TextDataType.Integer:
